...
function evalSafe(string: string): number {
    return new Function('return ' + string)();
}
function escapeRegExp(string: string) { //https://stackoverflow.com/questions/1144783/how-to-replace-all-occurrences-of-a-string-in-javascript
    return string.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); // $& means the whole matched string
}
function replaceAll(str: string, find: string, replace: string) {
    return str.replace(new RegExp(escapeRegExp(find), 'g'), replace);
}
const calculateY = (equation: string, x: number) => {
...
const PlotGraph = () => {
    ...
    const equation = (<HTMLInputElement>document.getElementById("equationTextfield")!).value;
    const points: number[][] = [];
...
const InitListeners = () => {
    document.getElementById("equationTextfield")!.oninput = () => { //to that there is always a (y =) at the start
        const text = (<HTMLInputElement>document.getElementById("equationTextfield")!).value;
        if (text.startsWith("y = ") == false) {
            (<HTMLInputElement>document.getElementById("equationTextfield")!).value = "y = ";
        }
    }
    ...
    let pinching = false;
    let previousScale = 1;
    graph.addEventListener('gesturechange', function($e: any) {
        pinching = true;
        ...
        if (SCALE >= 4) { X_INTERPOLATION_STEP = 0.25; }
        else if (SCALE >= 2) { X_INTERPOLATION_STEP = 0.5; }
        else if (SCALE >= 1) { X_INTERPOLATION_STEP = 1; }
        else { X_INTERPOLATION_STEP = 2; }
...
const MAIN = () => {
    ...
    (<HTMLInputElement>document.getElementById("equationTextfield")!).value = "y = x^2 - 100";
    InitListeners();
    PlotGraph();
}
